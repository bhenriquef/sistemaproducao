...
$num_total = $resultado = $con->query("select p.nome_peca, p.composicao_peca, p.referencia_peca, p.data_peca, g.tamanhop_grade,
                                    ...
                                    from peca p, grade g, tecido t, etapa e
                                    where p.id_peca = g.peca_id and p.etapa_id = e.id_etapa and t.id_tecido = p.tecido_id
                                    and e.id_etapa = '5'")->num_rows; //sql para pegar os valores
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                if($num > 0){
                    ...
                    $resposta = impResponsavelCostura($peca['id_peca']);
                    while ($responsavel = $resposta->fetch_assoc()){
                        ...
                        echo "<li><b>Responsavel: </b><u>".$responsavel['nome_usuario']."</u></li>
                                            ...
                                            <li><b> Data prevista de saida: </b><u>".date_format($datasaida,'d/m/y')."</u></li>
                                            <input hidden id='idcortador' name='idcortador' value='".$responsavel['id_usuario']."'>";
                    }
                    ...
                    echo "    </ul>
                               <form method='POST' action=''>
                                    <div class='row justify-content-center'>
                                        <div class='col-lg-4'>
                                            <input class='form-control btn btn-outline-success' type='submit' id='btnconcluir' name='btnconcluir' value='Concluir peça'>
                                            <input hidden id='idpeca' name='idpeca' value='".$peca['id_peca']."' type='text'>
                                        </div>
                                    </div>
                               </form>
                            </div>
                    ...
                    echo "<h2 class='text-center'>Não tem nada para ser costurado</h2>";
                }
                ?>
            </div>
        </div>
    </div>
    <form action="costura.php" method="GET">
        <nav>
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    <?php
                    if($pagina != 0){
                        echo "<a class='page-link' href='costura.php?pagina=0&i=0'><span aria-hidden='true'>&laquo</span></a>";
                    }
                    else{
                ...
                <li class="page-item">
                    <?php
                    if($pagina != 0){
                        echo "<a class='page-link' href='costura.php?pagina=".($pagina-1)."&i=".($i-1)."'> < </a>";
                    }
                    else{
                ...
                <li class="page-item">
                    <?php
                    if($pagina+1 < $num_total){
                        echo "<a class='page-link' href='costura.php?pagina=".(($num_paginas-1)*1)."&i=".($num_paginas-1)."'><span aria-hidden='true'>&raquo</span></a>";
                    }
                    else{
